...
@router.get(
    ...
    tags=["Comanda"],
    summary="Listar todas as comandas - opção de filtro e paginação - protegida por JWT"
)
@limiter.limit("moderate")
...
async def add_produto_to_comanda(
    ...
    try:
        ...
        AuditoriaService.registrar_acao(
            ...
            recurso_id=new_comanda_produto.id,
            dados_novos=new_comanda_produto,
            request=request
        )
...
# listrar produtos de uma comanda
@router.get(
    "/comanda/{id}/produtos",
...
@router.put(
    ...
    response_model=ComandaProdutosResponse,
    tags=["Comanda"],
    summary="Atualizar produto na comanda - quantidade e/ou valor - protegida por JWT e grupo 1"
)
@limiter.limit("restrictive")
async def update_comanda_produto(
    ...
    try:
        ...
        AuditoriaService.registrar_acao(
            ...
            acao="UPDATE",
            recurso="COMANDA_PRODUTO",
            recurso_id=comanda_produto.id,
            dados_antigos=dados_antigos_obj,
            dados_novos=comanda_produto,
            request=request
        )
